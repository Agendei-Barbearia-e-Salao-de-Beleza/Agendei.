...
export const SelectDateScreen: React.FC = () => {
  ...
  // Padding for empty days before 1st (May 1st 2026 is a Friday in real life, but in the mockup it's placed under 'Qui' which is Thursday)
  const emptyDays = [null, null, null, null]; // Dom, Seg, Ter, Qua
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0A0A0A] text-white font-sans relative overflow-x-hidden pb-24">
      ...
      <motion.div
        ...
        className="mx-5 bg-[#F59E0B] rounded-[2rem] p-6 shadow-xl"
      >
        <div className="text-center mb-5">
          <span className="text-white text-[13px] font-medium tracking-widest uppercase">
            Maio - 2026
          </span>
        </div>
        ...
        <div className="grid grid-cols-7 mb-4">
          {daysOfWeek.map((day, i) => (
            <div key={i} className="text-center">
              <span className="text-white text-[13px] font-bold">{day}</span>
            </div>
          ))}
        </div>
      ...
      >
        <motion.button
          onClick={() => {
            if (selectedDay) {
              navigate('/select-time', { state: { ...serviceState, date: `${selectedDay}/05/2026` } })
            }
          }}
